 ...
 */
static exfat_off_t s2o(const struct exfat* ef, exfat_off_t sector)
{
 ...
 * Cluster to sector.
 */
static exfat_off_t c2s(const struct exfat* ef, cluster_t cluster)
{
	if (cluster < EXFAT_FIRST_DATA_CLUSTER)
		exfat_bug("invalid cluster number %u", cluster);
	return le32_to_cpu(ef->sb->cluster_sector_start) +
		((exfat_off_t) (cluster - EXFAT_FIRST_DATA_CLUSTER) << ef->sb->spc_bits);
}
 ...
 * Sector to cluster.
 */
static cluster_t s2c(const struct exfat* ef, exfat_off_t sector)
{
...
static bool erase_raw(struct exfat* ef, size_t size, exfat_off_t offset)
{
