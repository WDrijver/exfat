...
const struct fs_object* const objects[] =
{
	&vbr,
	&vbr,
	&fat,
...
static int setup_spc_bits(int sector_bits, int user_defined, exfat_off_t volume_size)
{
	int i;

	if (user_defined != -1)
	{
		exfat_off_t cluster_size = 1 << sector_bits << user_defined;
